...
class Scraper:
    ...
    soup = BeautifulSoup(driver.page_source,'lxml')
    postings,follower,following = [e.text for e in soup.find_all('span',class_='_ac2a')]
    deschtml = soup.find('div',class_='_aa_c')
  
stats = [name,link,postings,follower,following,description,desclink]
ProfileStats(stats)

print([postings,follower,following,description])
link = driver.current_url
print(link)
